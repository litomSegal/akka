...
import akka.stream.scaladsl.Route
 ...
 * INTERNAL API
 */
private[akka] object RouteImpl {
  def props(settings: MaterializerSettings, outputCount: Int, routeLogic: Route.RouteLogic[Any]): Props =
    Props(new RouteImpl(settings, outputCount, routeLogic))
}
